...
from grab_symbol_unit_belonging_to_portfolio_id import (
    grab_symbol_unit_belonging_to_portfolio_id,
)
...
def show(portfolio_id, fiat_currency):
    try:
        ...
        symbol_unit_of_portfolio_id = grab_symbol_unit_belonging_to_portfolio_id(
            json_data, portfolio_id
        )

        # Grab each Symbols and each Units from the Symbol Unit pairs that belong to the portfolio ID
        each_symbol_from_pair, each_unit_from_pair = grab_each_symbol_unit_from_pair(
            symbol_unit_of_portfolio_id
        )
        ...
        symbol_same_sorting, quoted_prices = current_quoted_prices(
            show_json_string, fiat_currency
        )
...
if __name__ == "__main__":
    ...
    save_parser.add_argument(
        "portfolio_id",
        type=str,
        help="2nd arg of the save command requires a new or existing portfolio id",
    )
    save_parser.add_argument(
        "symbols_units",
        type=str,
        help="3rd arg of the save command requires symbol and quantity in this format Crypto Symbol=Units,Crypto Symbol=Units...",
    )
    ...
    show_parser.add_argument(
        "portfolio_id",
        type=str,
        help="2nd arg of the show command requires an existing portfolio id",
    )
    show_parser.add_argument(
        "fiat_currency",
        type=str,
        help="3rd arg of the show command requires a fiat currency symbol (e.g. USD, EUR)",
    )
